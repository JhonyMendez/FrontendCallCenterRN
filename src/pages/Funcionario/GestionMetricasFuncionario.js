...
import { usuarioAgenteService } from '../../api/services/usuarioAgenteService'; // 🔥 AGREGAR ESTA LÍNEA
...
export default function GestionMetricas() {
    ...
    const [nombreAgente, setNombreAgente] = useState('Cargando...'); // 🔥 NUEVO: Nombre del agente
    ...
    useEffect(() => {
        console.log('🔄 [DEBUG] useEffect agenteSeleccionado disparado');
        console.log('🔄 [DEBUG] agenteSeleccionado actual:', agenteSeleccionado);

        if (agenteSeleccionado) {
            console.log('✅ [DEBUG] Hay agente, cargando métricas y permisos...');
            cargarMetricas();
            cargarPermisosUsuario();
        } else {
            console.warn('⚠️ [DEBUG] No hay agente seleccionado');
        }
    }, [agenteSeleccionado]);

    // 🔥 AGREGAR este nuevo useEffect
    useEffect(() => {
        console.log('🔄 [DEBUG] useEffect inicial disparado (componentDidMount)');
        cargarPermisosUsuario(); // Cargar permisos al montar el componente
    }, []);
    ...
    const cargarAgentesReales = async (estadisticasAgentes) => {
        try {
            ...
            const idDepartamento = usuarioData?.id_departamento || usuarioData?.departamento?.id_departamento;
            ...
            console.log('🔥 Todos los agentes:', agentesArray);

            // 🔥 FILTRAR SOLO EL AGENTE DEL DEPARTAMENTO DEL USUARIO
            const agenteDelDepartamento = agentesArray.filter(agente => agente.id_departamento === idDepartamento);
            console.log('🔥 Agentes filtrados por departamento:', agenteDelDepartamento);
    ...
    const cargarAgentesDisponibles = async () => {
        try {
            ...
            // 🔥 FILTRAR SOLO LOS AGENTES DEL DEPARTAMENTO DEL USUARIO
            const agentesFiltrados = agentesArray.filter(agente => agente.id_departamento === idDepartamento);

            const resultado = agentesFiltrados.map(a => {
    ...
    const cargarPermisosUsuario = async () => {
        try {
            ...
            const usuarioData = await authService.getUsuarioActual();
            console.log('🔐 [DEBUG] usuarioData completo:', usuarioData);

            const idUsuario = usuarioData?.id_usuario;
            console.log('🔐 [DEBUG] idUsuario extraído:', idUsuario);

            if (!idUsuario) {
                console.error('❌ [DEBUG] No se pudo obtener ID de usuario');
                setPermisos({
                    puede_ver_metricas: false,
                    puede_exportar_datos: false,
                    cargando: false
            ...
            if (agenteSeleccionado) {
                ...
                console.log('🔐 [DEBUG] Respuesta de usuarioAgenteService:', permisosAgente);
            ...
            console.error('❌ [DEBUG] Error cargando permisos:', error);
    ...
    const cargarMetricas = async () => {
        try {
            ...
            // 🔥 GUARDAR NOMBRE DEL AGENTE (si existe un agente real)
            // 🔥 GUARDAR NOMBRE DEL AGENTE (si existe un agente real)
            console.log('🔥 agentesReales:', agentesReales);
            console.log('🔥 agentesDisp:', agentesDisp);

            if (agentesReales && agentesReales.length > 0) {
                console.log('✅ Usando agentes reales:', agentesReales[0]);
                setNombreAgente(agentesReales[0].nombre);

                // 🔥 AUTO-SELECCIONAR EL PRIMER AGENTE SI NO HAY UNO SELECCIONADO
                if (!agenteSeleccionado) {
                    console.log('🔥 [DEBUG] Auto-seleccionando agente ID:', agentesReales[0].id);
                    setAgenteSeleccionado(agentesReales[0].id);
                }
            } else if (agentesDisp && agentesDisp.length > 0) {
                // Si no hay agentes reales, usar disponibles
                console.log('ℹ️ Usando agentes disponibles:', agentesDisp[0]);
                setNombreAgente(agentesDisp[0].nombre);

                // 🔥 AUTO-SELECCIONAR EL PRIMER AGENTE SI NO HAY UNO SELECCIONADO
                if (!agenteSeleccionado) {
                    console.log('🔥 [DEBUG] Auto-seleccionando agente disponible ID:', agentesDisp[0].id);
                    setAgenteSeleccionado(agentesDisp[0].id);
                }
            } else {
                console.warn('⚠️ No se encontraron agentes');
                setNombreAgente('Sin agente asignado');
            }
    ...
    >
        <View style={metricasStyles.headerTop}>
            ...
            <View style={{ alignItems: 'flex-end' }}>
                {/* 🔥 MOSTRAR AGENTE AL LADO DE EXPORTAR */}
                ...
                <TouchableOpacity
                    style={[
                        metricasStyles.exportButton,
                        !permisos.puede_exportar_datos && { opacity: 0.5 } // 🔥 Opacidad si no tiene permiso
                    ]}
                    onPress={() => {
                        // 🔥 VALIDAR PERMISO ANTES DE ABRIR MODAL
                        if (!permisos.puede_exportar_datos) {
                            Alert.alert(
                                '🔒 Acceso Denegado',
                    ...
                    activeOpacity={0.7}
                    disabled={!permisos.puede_exportar_datos} // 🔥 Deshabilitar botón
                >
                    <LinearGradient
                    ...
                    >
                        <Ionicons
                            ...
                            size={22}
                            color={permisos.puede_exportar_datos ? "#667eea" : "#94a3b8"} // 🔥 Color gris si no tiene permiso
                        />
                    </LinearGradient>
                </TouchableOpacity>
            </View>
...
console.log('🎨 [DEBUG] RENDER - Estado actual de permisos:', permisos);
console.log('🎨 [DEBUG] RENDER - agenteSeleccionado:', agenteSeleccionado);
console.log('🎨 [DEBUG] RENDER - loading:', loading);
console.log('🎨 [DEBUG] RENDER - refreshing:', refreshing);
return (
                ...
                <Animated.View style={{ flex: 1, opacity: fadeAnim }}>
                    ...
                    >
                        ...
                        {loading && !refreshing ? (
                            ...
                            // 🔥 NUEVO: Mientras cargan los permisos
                            <View style={metricasStyles.loadingInline}>
                                <ActivityIndicator size="large" color="#667eea" />
                                <Text style={{ color: '#a29bfe', marginTop: 15, fontSize: 14 }}>
                        ...
                        ) : !permisos.puede_ver_metricas ? (
                            // 🔥 NUEVO: Sin permiso para ver métricas
                            <View style={{
                                flex: 1,
                                ...
                                <Text style={{
                                    ...
                                    No tienes permiso para ver las métricas de este agente.
                                </Text>
                                ...
                                <View style={{
                                    ...
                                    <Text style={{
                                        ...
                                        Contacta al administrador de tu departamento para solicitar el permiso "Ver métricas" en este agente.
                                    </Text>
                                </View>
                            </View>
                        ) : (
                            // 🔥 TIENE PERMISO: Mostrar métricas normalmente
                            <GestionMetricasCard
                                metricas={{
                                    ...metricas,
                                    onRecargarDatosDiarios: recargarDatosDiarios
                                }}
                                filtroActivo={filtroResumen}
